...
export const getMyAccounts = () =>
  client
    .get<{ account?: YodleeAccount[] }>('/yodlee/me/accounts')
    .then((r) => r.data.account ?? []);

export const getMyTransactions = (params?: { fromDate?: string; toDate?: string; top?: string }) =>
  client
    .get<{ transaction?: YodleeTransaction[] }>('/yodlee/me/transactions', { params })
...
export const updateMyAccount = (accountId: number, payload: { nickname?: string }) =>
  client
    .patch(`/yodlee/me/accounts/${accountId}`, payload)
    .then((r) => r.data);
